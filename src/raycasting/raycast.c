...
/*   Updated: 2024/01/24 13:58:25 by sbalk            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	raycast(t_cub *cub)
{
	...
	while (i < cub->win_size.x) // For each x axis pixel on the screen
	{
		...
		if (hit)
		{
			...
			if (side == 0) wallX = ray->pos.y + ray->perp_wall_dist * ray->dir.y;
			...
			for(int y = draw_start; y<draw_end; y++)
			{
				...
				texPos += stepTex;
				if (side == 0 && ray->pos.x > ray->map_check.x)
					color = get_pixel_color_int(&(cub->wall_textures[0]), texX, texY);
				else if (side == 0 && ray->pos.x < ray->map_check.x)
					color = get_pixel_color_int(&(cub->wall_textures[2]), texX, texY);
				else if (side == 1 && ray->pos.y > ray->map_check.y)
