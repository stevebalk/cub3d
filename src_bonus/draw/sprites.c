...
/*   Updated: 2024/01/28 16:58:07 by sbalk            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_rendering(t_cub *cub, int i)
{
	...
	stripe = cub->sc.draw_start.x;
	while (stripe < cub->sc.draw_end.x)
	{
		cub->sc.tex.x = (int)(256 * (stripe - (-cub->sc.size.x / 2 + cub->sc.sprite_screen_x)) * cub->sprite_textures[0].width / cub->sc.size.x) / 256;
		if (cub->sc.transform.y > 0 && stripe > 0 && stripe < cub->win_size.x && cub->sc.transform.y < cub->z_buffer[stripe])
		{
			...
			while (y < cub->sc.draw_end.y)
			{
				...
				color = get_pixel_color_int(&cub->sprite_textures[cub->sprites[i].id], cub->sc.tex.x, cub->sc.tex.y);
